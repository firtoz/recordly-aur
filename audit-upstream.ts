...
function parsePublishedPkgbuild(text: string): {
	...
	// AUR may still ship legacy "# Upstream MIT LICENSE" until the next push after the SPDX change.
	const licM =
		/^\t'([0-9a-f]{64})'.*# Upstream AGPL LICENSE/m.exec(text) ??
		/^\t'([0-9a-f]{64})'.*# Upstream MIT LICENSE/m.exec(text);
	if (!appM) die("Could not parse AppImage sha256 line (# AppImage) from AUR PKGBUILD");
	if (!licM) {
		die(
			"Could not parse LICENSE sha256 line (# Upstream AGPL LICENSE or # Upstream MIT LICENSE) from AUR PKGBUILD",
		);
	}
	return {
		pkgver,
